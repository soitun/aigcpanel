...
export const getGridPixel = (scale: number, frameCount: number) => {
    ...
    if (scale < 70) {
        // 1秒一格
        trackWidth = trackWidth / VIDEO_PREVIEW_FIXED_FPS;
    }
    if (scale < 30) {
        // 6秒一格
...
export const getSelectFrame = (
    ...
    return Math.round((offsetX + sizeDeltaRatio * size) / size);
};
/**
 * 时间轴画线
 * */
export const drawTimeLine = (
    ...
    if (focusPosition) {
        ...
        if (scale < 70) {
            // 一个单元格为 1 秒
            fStart = fStart / VIDEO_PREVIEW_FIXED_FPS;
            fCount = fCount / VIDEO_PREVIEW_FIXED_FPS;
        }
        if (scale < 30) {
            // 一个单元格为 6 秒
            fStart = fStart / 6;
