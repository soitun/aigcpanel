<template>
    ...
    >
        <TrackListIcon :listData="showTrackList as any" :offsetTop="startY" />
        <div
        ...
        >
            <TimeLine
                :start="startX"
                :scale="trackScale"
                :step="VIDEO_PREVIEW_FIXED_FPS"
                :focusPosition="store.selectResource as any"
            ...
            >
                <template v-if="showTrackList.length === 0">
                    <div
                        class="flex justify-center items-center h-20 m-auto w-2/3 dark:bg-gray-500 bg-gray-200 rounded-md text-sm border-dashed border-2 dark:border-gray-500 border-gray-200 hover:dark:border-blue-300 hover:border-blue-400"
                    >
                ...
                <template v-if="showTrackList.length !== 0">
                    <div
                        v-for="(
                            line, lineIndex
                        ) in store.dragData.fixLines.reduce(
...
import TimeLine from "./trackItem/TimeLine.vue";
import TrackLine from "./trackItem/TrackLine.vue";
import TrackListIcon from "./trackItem/TrackListIcon.vue";
import TrackPlayPoint from "./trackItem/TrackPlayPoint.vue";
...
const trackStyle = computed(() => {
    return {
        width:
            getGridPixel(trackScale.value, store.frameCount) + offsetLine.right,
    };
});
    ...
    return store.trackList.map((line, lineIndex) => {
        ...
        const newList = line.list.map((item) => {
            ...
            return {
                ...
                time:
                    "video" === line.type ? `${formatTime(time || 0).str}` : "",
                _raw: item,
            };
        });
        return {
...
function handlerSelectFrame(frame: number) {
    ...
    const startFrane =
        playFrame < 0
            ? 0
            : playFrame > store.frameCount
              ? store.frameCount
              : playFrame;
    playerStore.isPause = true;
    playerStore.frameIndex = startFrane;
}
...
const setScale = debounce(() => {
    ...
    store.trackScale = Math.min(Math.max(store.trackScale, 10), 100);
    maxDelta = 0;
}, 100);
...
function setFrameIndex(clientX: number) {
    ...
    playerStore.frameIndex = frameIndex;
}
...
function getPreciseOffset(element: HTMLElement) {
    const specifiedParent = document.getElementById(
        "track-container",
    ) as HTMLElement;
...
function getInsertLineInfo(): {
    ...
    const center =
        ...
        (dragElement!.offsetParent as any).offsetTop;
...
function getInsertInfo(): InsertInfo {
    ...
    const start =
        fixPosition.start ||
        getSelectFrame(left, store.trackScale, VIDEO_PREVIEW_FIXED_FPS);
    const end =
        fixPosition.end ||
        getSelectFrame(right, store.trackScale, VIDEO_PREVIEW_FIXED_FPS);
...
function adsorption(
    ...
    const minRightLine = lines[1].reduce(
        (r, item) => {
            return Math.abs(item.position - right) <
                Math.abs(r.position - right)
                ? item
                : r;
        },
        { position: Number.MAX_SAFE_INTEGER, frame: 0 },
...
function onMouseUp(event: MouseEvent) {
    if (dragElement) {
        store.dragData.moveX = event.pageX - store.dragData.dragPoint.x;
        store.dragData.moveY = event.pageY - store.dragData.dragPoint.y;
        if (store.dragData.moveX !== 0 || store.dragData.moveY !== 0) {
            const left = store.dragData.moveX + curCoord.left;
            const right = store.dragData.moveX + curCoord.right;
