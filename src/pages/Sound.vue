...
import SoundReplace from "./Apps/SoundReplace/SoundReplace.vue";
...
<template>
    <div class="pb-device-container bg-white h-full relative select-none flex">
        <div class="p-6 w-52 flex-shrink-0 border-r border-solid border-gray-100 overflow-x-hidden overflow-y-auto">
            ...
            <div class="p-2 rounded-lg mb-4 cursor-pointer" :class="tab === 'soundReplace' ? 'bg-gray-200' : ''"
                @click="tab = 'soundReplace'">
                <div class="text-base truncate">
                    <i class="iconfont icon-sound-replace w-6 inline-block"></i>
                    {{ t("声音替换") }}
                </div>
            </div>
        </div>
        <div class="flex-grow h-full overflow-y-auto">
            ...
            <SoundReplace v-if="tab === 'soundReplace'" />
        </div>
    </div>
</template>
