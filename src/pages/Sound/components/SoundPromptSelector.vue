...
<template>
    <div class="flex items-center">
        <div
            ...
            :class="
                disabled
                    ? 'cursor-not-allowed bg-gray-50 hover:bg-gray-50 text-gray-400'
                    : ''
            "
        >
            {{
                records.find((s) => s.id === props.modelValue)?.title ||
                $t("hint.selectTimbre")
            }}
        </div>
        <AudioPlayerButton
            ...
            :source="
                'file://' +
                records.find((s) => s.id === props.modelValue)?.content?.url
            "
        />
    </div>
    <SoundPromptDialog
        ref="soundPromptDialog"
        @update="doRefresh"
        @select="onSelect"
    />
</template>
