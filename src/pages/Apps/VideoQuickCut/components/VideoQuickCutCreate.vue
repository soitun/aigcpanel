...
const paramForm = ref<InstanceType<typeof VideoQuickCutParamForm> | null>(null);
...
const { clearDraft } = dataAutoSaveDraft(
    "VideoQuickCutCreate.formData",
    formData.value,
);

const doSubmit = async () => {
    const videoQuickCutValue = await paramForm.value?.getValue();
    if (!videoQuickCutValue) {
        return;
    }
