    ...
    <div class="rounded-xl shadow border p-4 mb-3">
        <div class="w-full flex flex-col items-center">
            <div class="flex items-center overflow-x-auto">
                <template
                    v-for="(step, idx) in stepDescriptions"
                    :key="step.key"
                >
                    <div
                    ...
                    >
                        <div
                            class="rounded-full w-9 h-9 flex items-center justify-center text-base font-bold transition-all duration-200 shadow"
                            :class="
                                'bg-white border-2 ' +
                                (idx === 0
                                    ? 'border-blue-400'
                        ...
                        <div
                            class="ml-2 text-base font-medium whitespace-nowrap"
                        >
                            {{ step.label }}
                        </div>
                        <div
                            v-if="idx < stepDescriptions.length - 1"
                            class="w-8 h-1 mx-2 rounded bg-gray-200"
                        ></div>
                    </div>
                </template>
            </div>
