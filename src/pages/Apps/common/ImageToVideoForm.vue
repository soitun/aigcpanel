...
const onServerUpdate = async (config: any) => {
    param.value = config.functions.imageToVideo?.param || [];
    modelConfig.value = config;
};

onMounted(async () => {
    const old = StorageUtil.getObject("ImageToVideoForm.formData");
...
watch(
    () => formData.value,
    async (value) => {
        StorageUtil.set("ImageToVideoForm.formData", value);
    },
    {
        deep: true,
    },
...
const getValue = async (): Promise<ImageToVideoParamType | undefined> => {
    const data: any = {};
    data.type = "ImageToVideo";
    data.serverKey = formData.value.serverKey;

    const server = await serverStore.getByKey(data.serverKey);
    if (!server) {
        Dialog.tipError(t("error.pleaseSelectImageToVideoModel"));
        return;
    }
    if (server.status !== EnumServerStatus.RUNNING) {
        Dialog.tipError(t("error.imageToVideoModelNotStarted"));
        return;
    }
    ...
    if (!data.param) {
        Dialog.tipError(t("error.imageToVideoParamInvalid"));
        return;
    }
...
const setValue = (data: Partial<ImageToVideoParamType>) => {
...
<template>
    <div class="mb-4">
        <div class="font-bold mb-2">
            ...
            {{ $t("app.imageToVideoConfig") }}
        </div>
        <div class="flex items-start min-h-8 max-w-lg w-full gap-1">
            <div class="pt-2">
                <a-tooltip :content="$t('app.imageToVideoModel')" mini>
