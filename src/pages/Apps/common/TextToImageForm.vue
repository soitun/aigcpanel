...
const getValue = async (): Promise<TextToImageParamType | undefined> => {
    ...
    if (!server) {
        Dialog.tipError(t("请选择文生图模型"));
        return;
    }
    if (server.status !== EnumServerStatus.RUNNING) {
        Dialog.tipError(t("文生图模型未启动"));
        return;
    }
...
<template>
    <div class="mb-4">
        ...
        <div class="flex items-start min-h-8 max-w-lg w-full gap-1">
            <div class="pt-2">
                <a-tooltip :content="$t('文生图模型')" mini>
                    <i class="iconfont icon-server"></i>
                </a-tooltip>
            </div>
            <div class="flex-grow flex flex-wrap gap-1">
                <div>
                    <ServerSelector v-model="formData.serverKey" @update="onServerUpdate" functionName="textToImage" />
                </div>
                <div class="">
