...
import GeneralModelCreate from "./components/GeneralModelCreate.vue";
...
useTaskChangeRefresh("GeneralModel", () => {
    doRefresh();
});
...
const doRefresh = async () => {
    const rawRecords = await TaskService.list("GeneralModel");
    records.value = mergeCheck(rawRecords);
};

// 历史任务中出现过的模型（按 serverName 去重）
...
<template>
    <div class="p-5">
        <PageHeader
            ...
            :desc="t('general.model.desc')"
        />
        <div>
            <GeneralModelCreate @submitted="doRefresh" />
            <ListerTop
                class="mt-4"
            ...
            >
                ...
                <template #actions>
                    <a-select
                        ...
                        :placeholder="t('general.filterModel')"
                        size="small"
                        allow-clear
                ...
                <div v-for="r in recordsForPage" :key="r.id">
                    ...
                    >
                        ...
                        <div v-if="r.result" class="mt-2">
                            <GeneralResultView
                                :result="r.result"
                                :result-def="(r as any).modelConfig?.resultDef"
                            />
                        </div>
            ...
            <m-empty v-else :text="t('general.noTask')" />
        </div>
    </div>
</template>
