...
export const SubtitleTts: TaskBiz = {
    runFunc: async (bizId, bizParam) => {
        ...
        jobResult.ParseSrt = jobResult.ParseSrt || {
            status: "queue",
            records: [],
        };
        ...
        if (jobResult.step === "SoundGenerate") {
            ...
            if (!jobResult.SoundGenerate.records) {
                jobResult.SoundGenerate.records =
                    jobResult.ParseSrt.records.map((item) => ({
                        ...item,
                        audio: "",
                    }));
            }
        ...
        if (jobResult.step === "Combine") {
            ...
            try {
                // 计算总时长
                const totalDuration =
                    jobResult.ParseSrt.records.length > 0
                        ? Math.max(
                              ...jobResult.ParseSrt.records.map((r) => r.end),
                          )
                        : 0;
    ...
    successFunc: async (bizId, bizParam) => {
        ...
        if (jobResult.step === "End") {
            ...
            $mapi.log.error(
                "SubtitleTts.successFunc: unknown jobResult.step",
                jobResult.step,
            );
        }
    },
