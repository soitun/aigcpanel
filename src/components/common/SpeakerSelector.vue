            ...
            <div>
                ...
                <div class="px-3 pb-2">
                    <div class="inline-block bg-gray-100 rounded-lg px-1 pt-1">
                        ...
                        <a-button
                            ...
                            class="mb-1 mr-1"
                            :type="
                                filterLang === l.name ? 'primary' : undefined
                            "
                ...
                >
                    ...
                    <div v-for="s in filterRecords" :key="s.name">
                        <div
                            class="border mb-3 p-3 flex items-start rounded-lg shadow margin-top"
                        >
                            <div class="flex-grow">
                                <div class="mb-2 flex items-center">
                                    <img
                                        :src="s.cover"
                                        class="h-4 rounded-full shadow w-4 mr-1"
                                    />
...
const langs = computed(() => {
    ...
    const result = Object.entries(langMap).map(([name, count]) => ({
        name,
        count,
    }));
    const sortOrder = { 中文: 1, 英文: 2, 中文方言: 3, 其他: 4 };
    result.sort(
        (a, b) => (sortOrder[a.name] || 999) - (sortOrder[b.name] || 999),
    );
    return result;
});
...
const filterRecords = computed(() => {
    return props.speakers.filter((s) => {
        ...
        if (filterLang.value && !s.langs.includes(filterLang.value))
            return false;
