...
import { t } from "../lang";
import { SoundGenerateReplaceContent } from "../pages/Video/components/config/replaceContent";
...
export const serverSoundAsr = async (
    ...
    if (option.cache) {
        const cacheRecords = await $mapi.file.cacheGet<any[]>({
            soundAsr,
            audio,
        });
        if (cacheRecords) {
...
export const replaceSoundGenerateText = async (
    text: string,
): Promise<string> => {
    if (!text) {
        return text;
...
export const serverTextToImage = async (
    ...
    const server = await serverStore.getByNameVersion(
        textToImage.serverName,
        textToImage.serverVersion,
    );
    if (!server) {
...
export const serverImageToImage = async (
    ...
    const server = await serverStore.getByNameVersion(
        imageToImage.serverName,
        imageToImage.serverVersion,
    );
    if (!server) {
