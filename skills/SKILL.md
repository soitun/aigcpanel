...
AigcPanel Pro 内置 HTTP 接口服务，默认监听 `http://localhost:60000`，提供模型列表、模型调用和结果查询能力。
...
```
GET http://localhost:60000/api/model/list
```
...
```
POST http://localhost:60000/api/model/call
...
```
GET http://localhost:60000/api/model/query?taskId=m3x7k2abc
```
