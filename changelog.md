...
- **文生视频**：输入文字描述即可生成视频
- **图生视频**：上传图片加文字描述，让图片动起来生成视频
- **通用模型工具**：新增通用分组，可调用更多类型的 AI 模型
- **文件自动保存**：任务完成后结果文件自动保存到软件，删除或重装模型后仍可查看和导出
- **音色导出**：音色管理支持导出音色文件到本地
- **菜单记忆**：左侧菜单位置自动记忆，切走再切回时停留上次位置
- **多图上传**：支持同时上传多张图片，可预览和逐张删除

### ⚡ 功能优化

- **ComfyUI 自启动**：AI 创作工具改为自动启动模式，使用时自动开启，空闲时自动关闭，无需手动管理
- **模型统一命名**：AI 模型服务统一称为"服务(server)"，界面和操作更一致
- **直播功能升级**："数字人直播"更名为"视频直播"，形象选择更灵活
- **界面简化**：AI 模型列表操作整合到"更多"菜单，界面更清爽
- **参数输入优化**：多行文本框标签移到上方，布局更清晰
- **模型能力标识**：模型管理增加能力标识（视觉识别、工具调用），添加/编辑时可设置
- **工作流图标升级**：工作流节点图标支持自定义颜色和样式

### 🔧 问题修复

- **模型状态修复**：修复模型启动失败后状态显示不正确的问题
- **文件上传校验**：音频文件上传增加格式校验，防止误传视频文件
- **任务稳定性**：修复自启动模型偶发任务卡住的问题
- **音频选择修复**：数字人视频生成时音频文件选择增加格式校验
